...
const char *response_body = "Hi hi there!\n";

std::string build_response(uint16_t status_code, std::string_view body) noexcept
{
    std::stringstream ss;
            ...
            if (FD_ISSET(i, &working_set))
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        /**********************************************/
                        std::string response =
                            build_response(200, response_body);
                        rc = (int)send(i, response.c_str(), response.size(), 0);
                        if (rc < 0)
                        {
                            perror("  send() failed");
                            close_conn = TRUE;
